...
trait DatabaseWriter {
  val props = Map(
    ...
    //modiificar jdbc:postgresql://postgres:5432/tracking -> jdbc:postgresql://localhost:5432/tracking para pruebas en local
    , JDBCOptions.JDBC_URL -> "jdbc:postgresql://localhost:5432/tracking"
    , "user" -> "docker"
    , "password" -> "docker"
